...
public:
	void set_admin_user(std::string a) {
		admin_username = a;
...
public:
	...
	void view_student_timetable() {
		...
		while (!a.eof())
		{
			a >> L_start >> L_end >> teacher_name >> teacher_namel >> sect;
	...
	void remove_student_timetable() {
		...
		file.open("TimeTable.dat", std::ios::in);
		temp.open("TimeTabletemp.dat", std::ios::out);
		std::cout << "Enter Section to Remove (i.e. 2G-1)\n";
		cin >> section;
...
public:
	...
	void view_teacher()
	{
		...
		cout << "Fst_name   | " << " Lst_name   | " << "Experience  | " << " Teacher ID " << " | " << "    Phone_no    | " << "   Subject |" << "\n";
		cout << "______________________________________________________________________________________\n";
		while (!file_obj.eof())
		{
...
public:
	...
	void view_student_fee() {
		ifstream file;
		file.open("StudentFee.dat", ios::in);
		if (!file) {
			std::cout << "File Connection Error\n";
		}
		else {
...
public:
	void add_result_student() {
		...
		std::cout << "1. Moke Exam Marks\n";
		std::cout << "2. Pre_Board Marks\n";
		...
		if (x == 1) {
			while (!file_obj.eof()) {
				...
				if (id_student == search_id_student) {
					std::cout << "Enter Student's Moke Exam Marks: ";
		...
		if (x == 2) {
			while (!file_obj.eof()) {
				file_obj >> student_fname >> student_lname >> id_student >> moke_marks >> pre_boared_marks >> class_marks;
				if (id_student == search_id_student) {
					std::cout << "Enter Student's Pre Board Exam Marks: ";
...
public:
	...
	void view_StudentAttendence()
	{
		...
		std::cout << "                                 |  Date     " <<" | Student i'd | "<< " Status | " << "\n";
		std::cout << "                                 ________________________\n";
		while (!file_obj.eof())
		{
			file_obj >> day >> month >> year >> teacher_ID >> present_status;
			...
			std::cout << "                                 | " << day << "/" << month << "/" << year << "  |      "<< teacher_ID<<"  |      " << present_status << "   |    "<< "\n";
